...
if ( ! class_exists( 'Kelpie_Walker_Comment' ) ) {
	/**
	 * CUSTOM COMMENT WALKER
	 * A custom walker for comments, based on the walker in Twenty Nineteen.
	 */
	class Kelpie_Walker_Comment extends Walker_Comment {
		...
		protected function html5_comment( $comment, $depth, $args ) {
			...
			<<?php echo $tag; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static output ?> id="comment-<?php comment_ID(); ?>" <?php comment_class( $this->has_children ? 'parent' : '', $comment ); ?>>
				<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
					<footer class="comment-meta">
						...
						<div class="comment-metadata">
							<a href="<?php echo esc_url( get_comment_link( $comment, $args ) ); ?>">
								...
								$comment_timestamp = sprintf( __( '%1$s at %2$s', 'kelpie' ), get_comment_date( '', $comment ), get_comment_time() );
								?>
					...
					$by_post_author = kelpie_is_comment_by_post_author( $comment );
